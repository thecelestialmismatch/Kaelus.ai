...
[2026-03-16] | TASK: Gap 6 PDF reports | PATTERN: Dynamic import (await import()) in the API route for jsPDF keeps it out of the server bundle unless needed — critical for Vercel cold starts | ANTI-PATTERN: Don't import jsPDF at the top of the route file — it adds ~500KB to the server bundle even for non-PDF requests
[2026-03-16] | TASK: Gap 7 landing copy | PATTERN: Use whileInView instead of initial animate for landing page sections — components below the fold start invisible and animate in when scrolled to, which is correct | ANTI-PATTERN: Don't use motion.div animate on components not initially in view — they animate immediately on page load, not on scroll
[2026-03-16] | TASK: Gap 8 onboarding + email | PATTERN: Welcome email fires from handleLaunch with try/catch — email failure must never block onboarding completion | ANTI-PATTERN: Don't await email sends in the critical path without a catch; a Resend API error would leave the user stuck on the launch screen
[2026-03-16] | TASK: Gap 9 CMMC patterns | PATTERN: CAGE codes need context ("CAGE code 1ABC2") not just the 5-char pattern — bare 5-char alphanumeric would generate massive false positives in normal text | ANTI-PATTERN: Don't write DoD contract number regex without anchoring to the known prefix structure (letter+digits+letter format) — bare number patterns hit too many false positives
[2026-03-16] | TASK: Gap 10 docs | PATTERN: Default the docs landing to "Quickstart" not the raw API reference — first-time visitors need the "why" and working code before the spec | ANTI-PATTERN: Don't make the API reference page the docs homepage — it's intimidating for evaluators and useless without context
