...
const LAYERS = [
  {
    ...
    tags: [
      { label: "ChatGPT.com", color: "text-indigo-400" },
      { label: "Cursor / VS Code", color: "text-indigo-400" },
      { label: "Internal Apps", color: "text-indigo-400" },
      { label: "Slack Bots", color: "text-indigo-400" },
    ],
  },
  {
    num: "LAYER 2",
            ...
            {LAYERS.map((layer, i) => {
              ...
              return (
                <div key={i}>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-left">
                          <div className="text-[11px] font-bold font-mono text-white/30 tracking-wider mb-1">
                            {layer.num}
                          </div>
                      ...
                      <ChevronDown
                        className={`w-5 h-5 text-white/30 flex-shrink-0 transition-transform duration-300 ${isOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                            ...
                            <div className="pt-4 border-t border-white/[0.05]">
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                {layer.tags.map((tag) => (
                                  <div
                                    key={tag.label}
                                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-black/25 border border-white/[0.04] text-sm"
                                  >
                  ...
                  {i < LAYERS.length - 1 && (
                    <div className="flex justify-center py-1 text-white/[0.12] text-lg">▾</div>
                  )}
                </div>
              );
            })}
