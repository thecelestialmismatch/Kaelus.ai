...
function addPageHeader(doc: jsPDF, pageNum: number, orgName: string, generatedAt: string) {
  ...
  doc.text("Kaelus.ai — AI Compliance Firewall", MARGIN, 10);
...
export function generateCompliancePDF(data: ReportData): Buffer {
  ...
  doc.setTextColor(DARK_GREY);
  const attestation = [
    "This report was generated by Kaelus.ai, an AI compliance firewall operating under CMMC Level 2",
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (i > 1) {
      ...
      doc.setTextColor(MID_GREY);
      doc.setFont("helvetica", "normal");
      const footerText = `Generated by Kaelus.ai  |  Merkle Root: ${data.integrity.merkle_root ?? "N/A"}  |  Tamper-evident audit trail`;
