...
export default function ContactPage() {
  ...
  const inputCls = "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder-white/20 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30 transition-all duration-200";
        ...
        <div className="max-w-4xl mx-auto grid sm:grid-cols-3 gap-5">
          {cards.map((c, i) => (
            <FadeIn key={c.label} delay={i * 100}>
              <div className="border border-slate-200 bg-white rounded-2xl p-6 text-center hover:border-white/[0.12] hover:bg-slate-100 transition-all duration-200 cursor-pointer">
                <c.icon className={`w-6 h-6 ${c.color} mx-auto mb-3`} />
                <p className="text-sm font-semibold text-slate-900 mb-1">{c.label}</p>
                <p className="text-sm text-slate-600 font-medium mb-1">{c.value}</p>
                <p className="text-xs text-slate-600 dark:text-slate-400">{c.sub}</p>
              </div>
            </FadeIn>
          ))}
        </div>
              ...
              <AnimatePresence mode="wait">
                {submitted ? (
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-12">
                    ...
                    <p className="text-slate-600 dark:text-slate-400 mb-8">We will respond within 4 business hours.</p>
                  ...
                  <motion.div key="form" initial={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
                      <div>
                        <label className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider block mb-1.5">Name *</label>
                      ...
                      <div>
                        <label className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider block mb-1.5">Email *</label>
                      ...
                      <div>
                        <label className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider block mb-1.5">Company</label>
                      ...
                      <div>
                        <label className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider block mb-1.5">Subject</label>
                      ...
                      <div>
                        <label className="text-xs font-medium text-slate-600 dark:text-slate-400 uppercase tracking-wider block mb-1.5">Message *</label>
          ...
          <FadeIn delay={150}>
            <div>
              ...
              <div className="space-y-3">
                {faqs.map((f, i) => (
                  <div key={i} className="border border-slate-200 bg-white rounded-2xl overflow-hidden hover:border-white/[0.12] transition-colors duration-200">
        ...
        <div className="max-w-[1200px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">AI compliance firewall. Real-time interception. Encrypted quarantine. Immutable audit trails.</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.15em] text-slate-600 dark:text-slate-400 font-semibold mb-4">Product</p>
              <div className="space-y-2.5">
                <Link href="/features" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Features</Link>
                <Link href="/pricing" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Pricing</Link>
                <Link href="/command-center" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Command Center</Link>
                <Link href="/changelog" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Changelog</Link>
              </div>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.15em] text-slate-600 dark:text-slate-400 font-semibold mb-4">Compliance</p>
              <div className="space-y-2.5">
                <span className="block text-sm text-slate-600 dark:text-slate-400">SOC 2</span>
                <span className="block text-sm text-slate-600 dark:text-slate-400">GDPR</span>
                <span className="block text-sm text-slate-600 dark:text-slate-400">EU AI Act</span>
                <span className="block text-sm text-slate-600 dark:text-slate-400">HIPAA</span>
                <span className="block text-sm text-slate-600 dark:text-slate-400">CMMC / NIST 800-171</span>
              </div>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.15em] text-slate-600 dark:text-slate-400 font-semibold mb-4">Company</p>
              <div className="space-y-2.5">
                <Link href="/about" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">About</Link>
                <Link href="/docs" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Documentation</Link>
                <Link href="/contact" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Contact</Link>
                <Link href="/signup" className="block text-sm text-slate-600 dark:text-slate-400 hover:text-slate-500 transition-colors cursor-pointer">Get Started</Link>
              </div>
            </div>
          </div>
          <div className="border-t border-slate-200 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-700 dark:text-slate-300">
              <Link href="/privacy" className="hover:text-slate-600 dark:text-slate-400 transition-colors cursor-pointer">Privacy Policy</Link>
              <Link href="/terms" className="hover:text-slate-600 dark:text-slate-400 transition-colors cursor-pointer">Terms of Service</Link>
              <Link href="/docs" className="hover:text-slate-600 dark:text-slate-400 transition-colors cursor-pointer">Security</Link>
            </div>
          </div>
        </div>
      </footer>
